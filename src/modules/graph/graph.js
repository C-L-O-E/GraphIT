...
async function test(){ 
  // Erstelle eine Instanz des Graphen und füge sie in das div mit der ID "graphContainer" ein
  const graph = new Graph('graphView',800,600);
  
  // Füge einige Knoten und Kanten hinzu
  ...
  const edgeAB = new Edge(0, 1);
  graph.edges.push(edgeAB);
  
  const edgeAC = new Edge(0, 2);
  graph.edges.push(edgeAC);
  
  const edgeBC = new Edge(1, 2, true);
  graph.edges.push(edgeBC);
  
  const edgeCD = new Edge(2, 3);
  graph.edges.push(edgeCD);

  const edgeCC = new Edge(3, 3);
  graph.edges.push(edgeCC);

  const edgeCH = new Edge(3, 7,true);
  graph.edges.push(edgeCH);

  const edgeCG = new Edge(3, 6,true);
  graph.edges.push(edgeCG);

  
  // Füge Text zu den Kanten hinzu
  edgeAB.setText('Edge AB');
  edgeAC.setText('Edge AC');
  edgeBC.setText('Edge BC');
  edgeCD.setText('Edge CD');
  
  graph.draw();
  // Starte die Tiefensuche
  await graph.tiefensuche();
  
  graph.resetColor();

  // Starte die Breitensuche
  await graph.breitensuche();
}
  //test();
